...
//  而真实设备上大部分项目都带网格标定 —— 用旧备份恢复,这些四角标定全部静默丢失,
//  没有任何测试会红。
//
//  新格式初版还漏了 `purchaseRecords` 和 `partsSheet`(两者旧 JSON 里都有) ——
//  多零件图纸漏掉的后果是:换新手机恢复后,每个零件的框、逐格色号、拼豆板摆位全没了。
//  两次都说明同一件事:**"我记得都写了"不是保证,逐字段比对才是。**
//
//  所以下面刻意用**全字段非默认值**的 fixture。新增字段时请同步扩充
//  `makeFullyPopulatedManifest`。
//
//  ## 这组测试**覆盖不到**什么(别高估它)
//
//  它只 round-trip `BackupArchiveManifest` 这个结构体本身,走的是编译器合成的 Codable。
//  真正出过事的那两次都在**接线**上,不在这一层:取数有没有取这一列、写出器有没有写这个
//  文件、`validate` 的遍历数组有没有带上它、`apply` 有没有读回来 —— 这四处任一漏掉,
//  这里照样全绿。要覆盖接线需要一条 `ModelContainer → write → validate` 的端到端测试。
...
final class BackupArchiveRoundTripTests: XCTestCase {
    ...
    func testManifestRoundTripPreservesEveryField() throws {
        ...
        // 全部 blob 引用 —— patternGrid 是旧格式漏掉的那个，partsSheet 是新格式初版
        // 漏掉的那个（旧 JSON 反而有），两个都必须在场

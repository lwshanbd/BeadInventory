...
enum PartsCellClassifier {
    ...
    private static func assignIdentities(
        ...
        // **图纸自己写了用色表，就优先在这张表里选。**
        //
        // 更早的一版是「图例里 ΔE ≤ 25 才用图例，否则去几百色的全色库里找最近的」，
        // 结果图纸上明明只有十来种豆子，却认出一堆表上压根没有的色号（全色库里
        // 相邻色号的色差中位数才 5 点出头，总有一个「更近」的）。所以门槛收到了
        // `legendMissDeltaE`(16)：图例里有一个像的，就用图例的，不再去全色库里挑更像的。
        //

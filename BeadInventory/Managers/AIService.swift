        ...
        case .table:
            ...
            case .kaka:
                ...
                - 其中某一行是卡卡品牌的色号，卡卡色号有三种前缀：
                  ...
                  - R系列：R1, R2, R3, R20 等
                  - 格式统一为：单个字母(B/P/R) + 数字
                  - 数字部分不带前导零，例如应输出 B2 而不是 B02、P5 而不是 P05
                ...
                - 卡卡色号不带前导零：输出 B2 而不是 B02，输出 P5 而不是 P05
                ...
                - 图片可能有水印干扰，请仔细辨认文字
                """
                let user = "请识别这张色号表格图片，提取所有卡卡色号和对应的数量。卡卡色号有B、P、R三种前缀（如B3, B257, P12, R5），格式为单个字母+数字，数字不带前导零（输出 B2 而非 B02）。先判断表格结构，找到卡卡行和数量行。如有多个表格区块请全部识别。返回内容必须且只能是严格JSON，顶层必须是 {\"items\":[...]}，不要返回裸数组，不要使用代码块，quantity必须是整数。"
                return (system, user)

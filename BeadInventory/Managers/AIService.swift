...
class AIServiceManager: ObservableObject {
    ...
    private func recognizeWithOpenAI(base64Image: String, mediaType: String, mode: RecognitionMode) async throws -> [AIRecognizedItem] {
        ...
        case .table:
            ...
            - 表格最左侧一列是行标签，用于标识每一行的含义（如"MARD"、"数量"、"vivid"、"漫漫"等）
            - 请根据最左侧的行标签来判断哪一行是MARD色号、哪一行是数量
            - MARD色号格式如：F8, A17, B195, DH01, IC09等（通常是字母+数字）
            - 数量行的标签可能是"数量"、"总数"、"需要"等类似含义的文字
            - 其他品牌行（vivid, 漫漫, 卡卡等）请忽略

            特殊情况 - 双区块表格：
            - 当颜色数量较多时，图片中可能出现上下两个独立的表格区块
            - 每个区块都有自己的行标签、品牌行和数量行

...
//  ## 一致性模型:逐记录一致(产品已裁决)
//
//  **保证的**:每个项目的全部 blob 由**一次 fetch**取回(`ProjectImageLoader.blobs(for:)`),
...
//
//  初版实现其实**连逐记录一致都没做到** —— 一个项目的几张图分几次取,来自几个不同时刻,
...
struct ArchivedProject: Codable {
    ...
    /// 多零件图纸（`BeadPartsSheet` 的 JSON 字节）。
    ///
    /// 本格式初版没有这一节 —— 那会是与 `patternGrid` 完全同类的静默数据丢失：
...
enum BackupArchiveWriter {
    ...
    ///   写完即释放；任何时刻内存里最多**一个项目的全部 blob**，与项目总数无关。
    /// - 每写完一条检查一次取消。
    ...
    static func write(
        ...
        for project in snapshot.projects {
            ...
            // 这才让「逐记录一致」名副其实。分几次取的话，同一个项目的几张图会来自几个
            ...
            // partsSheet：同样搬原始字节，不解码再编码 —— 快照可能是别的设备用更新的版本
            // 写的，解码会把本版本不认识的字段悄悄抹掉，解码失败还会整份丢弃。
            if let partsData = blobs.partsSheetData {
...
enum BackupArchiveReader {
    ...
    static let maxBlobCount = 500_000          // 每项目最多 5 个
    ...
    static func validate(archiveAt root: URL) throws -> ValidationReport {
        ...
        for project in manifest.projects {
            for ref in [project.thumbnail, project.finishedImage, project.displayThumbnail,
                        project.patternGrid, project.partsSheet].compactMap({ $0 }) {
...
extension BackupArchiveReader {
    ...
    static func apply(
        ...
        for (index, p) in m.projects.enumerated() {
            ...
            // 说成"原有数据未被改动"。
            let thumbnail: Data?, finished: Data?, display: Data?, gridData: Data?, partsData: Data?
            do {
                ...
                partsData = try p.partsSheet.map { try readBlob($0, root: report.root) }
            } catch {
